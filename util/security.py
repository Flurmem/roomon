...
def validar_usuario_logado(request: Request) -> Pessoa | None:
    try:
        token = request.cookies["auth_token"]
        if token.strip() == "":
            return None
        usuario = pessoaRepo.obterUsuarioPorToken(token)
        return usuario
    except KeyError:
        return None

def validar_crianca_logado(request: Request) -> Pessoa | None:
  try:
      token = request.cookies["auth_token_crianca"]
      if token.strip() == "":
          return None
      usuario = pessoaRepo.obterCriancaPorToken(token)
      return usuario
  except KeyError:
      return None
